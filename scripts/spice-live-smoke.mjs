...
const maxLastIngressAckMs = Number(process.env.SPICE_SMOKE_MAX_LAST_ACK_MS ?? '50');
const maxQxlBandSentinelTrips = Number(
  process.env.SPICE_SMOKE_MAX_QXL_SENTINEL_TRIPS ?? '0',
);
const requireQxlCoverage = process.env.SPICE_SMOKE_REQUIRE_QXL_COVERAGE !== '0';
const maxCpuMirrorSyncs = Number(process.env.SPICE_SMOKE_MAX_CPU_MIRROR_SYNCS ?? '0');
...
function recentMinIngressDepth(samples) {
  const recentSamples = samples.slice(-5);
  if (recentSamples.length === 0) {
    return Infinity;
  }
  return Math.min(
    ...recentSamples.map((sample) => numberValue(sample.performance?.ingressQueueDepth)),
  );
}

function assertSmoke(samples, consoleErrors) {
  if (samples.length === 0) {
  ...
  if (typeof perf.firstFrameMs !== 'number') {
    throw new Error(`No first frame by end of smoke. stage=${last.stage ?? 'unknown'}`);
  }

  if (typeof perf.p95PacketToPresentMs === 'number' && perf.p95PacketToPresentMs > maxP95Ms) {
    throw new Error(
      `p95 packet-to-present too high: ${perf.p95PacketToPresentMs}ms > ${maxP95Ms}ms`,
    );
  }
  ...
  const ackCount = numberValue(perf.ingressAckCount) + numberValue(perf.ingressAckSyncCount);
  ...
  const firstAckCount =
    numberValue(firstPerf.ingressAckCount) + numberValue(firstPerf.ingressAckSyncCount);
  const packetDelta =
...
function assertVisualSummary(summary) {
  if (!visualCheck) {
    return;
  }
...
async function captureCanvasVisualSummary(page) {
  if (!visualCheck) {
    return null;
  }
  ...
  return page.evaluate(async (base64) => {
    ...
    const loaded = new Promise((resolve, reject) => {
      ...
      image.onerror = () => reject(new Error('Unable to decode canvas screenshot.'));
    });
...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(
      () => Boolean(window.__spiceDebug?.nextSnapshot),
      null,
    ...
    const samples = await page.evaluate(async (durationSeconds) => {
      ...
      while (performance.now() < deadline) {
        const snapshot = await Promise.race([
          window.__spiceDebug.nextSnapshot(1000),
    ...
    if (recentMinIngressDepth(samples) > maxIngressQueueDepth && drainGraceMs > 0) {
      const drainSamples = await page.evaluate(async ({ graceMs, maxDepth }) => {
        const output = [];
        const deadline = performance.now() + graceMs;
        while (performance.now() < deadline) {
          const snapshot = await Promise.race([
            window.__spiceDebug.nextSnapshot(500),
      ...
      JSON.stringify(
        {
          ...
          qxlBandSentinelTrips: last.performance?.qxlBandSentinelTrips ?? null,
          ...
          wholeScreenBlockCount: last.performance?.wholeScreenBlockCount ?? null,
