...
export class GpuRopCopyPass {
  ...
  initialize() {
    ...
    const shader = this.device.createShaderModule({ code: ropCopyShader() });
  ...
  render(
    ...
    destSnapshotTexture: GpuTexture,
    targetView: unknown,
    dest: PresentRect,
    srcUv: PresentRect,
    ...
    this.device.queue.writeBuffer?.(
      ...
      new Float32Array([
        ...
        srcUv.right,
        srcUv.bottom,
        Math.max(0, Math.floor(ropDescriptor)),
        0,
        0,
        0,
      ]),
    ...
    const bindGroup = this.device.createBindGroup({
      ...
      entries: [
        ...
        { binding: 1, resource: destSnapshotTexture.createView() },
...
@group(0) @binding(1) var destTex: texture_2d<f32>;
@group(0) @binding(2) var imageSampler: sampler;
@group(0) @binding(3) var<uniform> params: Params;

struct VertexOut {
  @builtin(position) position: vec4<f32>,
  @location(0) srcUv: vec2<f32>,
  @location(1) destUv: vec2<f32>,
};
...
fn vsMain(@builtin(vertex_index) vertexIndex: u32) -> VertexOut {
  ...
  let uvMin = params.srcUvRect.xy;
  let uvMax = params.srcUvRect.zw;
  var out: VertexOut;
  out.position = vec4<f32>(positions[vertexIndex], 0.0, 1.0);
  out.srcUv = uvMin + baseUv[vertexIndex] * (uvMax - uvMin);
  out.destUv = baseUv[vertexIndex];
  return out;
}

fn colorToByte(color: vec3<f32>) -> vec3<u32> {
  return vec3<u32>(round(clamp(color, vec3<f32>(0.0), vec3<f32>(1.0)) * 255.0));
}

fn byteToColor(color: vec3<u32>) -> vec3<f32> {
  return vec3<f32>(color) / 255.0;
}

fn applyDrawCopyRop(rop: u32, sourceInput: vec3<u32>, destInput: vec3<u32>) -> vec3<u32> {
...
fn fsMain(input: VertexOut) -> @location(0) vec4<f32> {
  let source = colorToByte(textureSample(sourceTex, imageSampler, input.srcUv).rgb);
  let dest = colorToByte(textureSample(destTex, imageSampler, input.destUv).rgb);
  let rop = u32(round(params.rop.x));
  let color = applyDrawCopyRop(rop, source, dest);
  return vec4<f32>(byteToColor(color), 1.0);
}
`;
}
